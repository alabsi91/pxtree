...
`<target>` is a URL, a `localhost:3000`-style host (gets `http://`), or a file path (becomes `file://`, resolved from cwd). A `?query` or `#fragment` after a file path stays on the URL and is not part of the file name, unless a file with the whole name exists. A directory target stops with `target is a directory: <target>`, exit 1.
...
| ------------------------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--viewport <WxH[,WxH...]>`          | `1280x800` | One measurement per viewport. `x` or `X`. Width and height must be positive. A width alone gets a matching height: 390→844, 768→1024, 820→1180, 1024→768, 1280→800, 1440→900, 1920→1080, and 800 for any other width.                                                                                                                              |
...
| `--report <detail>`                  | `tree`     | How much of the measurement prints: `tree` everything, `findings` the tree cut down to the lines that carry a finding plus the bare name lines of their ancestors (5.1), `summary` no tree, `changes` the facts line and since last run, `none` the facts line only. `none` still measures while the cache is on, and skips the measurement with `--no-diff` and no `--element`. `none` needs `--aria` or `--screenshot` (exit 1). |
| `--aria`                             | off        | After the report, the aria tree of the page (section 5.1), or of each `--element` match, always with its whole subtree.                                                                                                                                                                                                                            |
...
| `--timeout <ms>`                     | `30000`    | Budget for reaching DOMContentLoaded. The `load` event and a quiet network have their own short caps (4.9), then pxtree measures anyway. The in-page measurement gets what is left of the budget after loading (4.9 step 8).                                                                                                                        |
...
| 1    | Bad usage (unknown flag, bad viewport string, script file missing, directory target, `--report none` without `--aria` or `--screenshot`). |
...
measurement timed out after 2918 ms
...
export function getTargetUrl(target: string): string; // the URL measure loads for a target (2.1)
...
export type { SessionOptions, Session, MeasureOptions, MeasureResult, RunResult, FontFallback, FormatOptions, ReportDetail } from './types.ts';
```
...
| `sr-only`              | border box at most 1x1; or overflow not `visible`, no background or border, and a content box at most 1x1, or above 0 and at most 1 px on either axis (a 1x44 nav label; a 0 px axis alone is a collapsed box, not sr-only); or `clip` computes to `rect(0px, 0px, 0px, 0px)`; or `clip-path` is `inset(50%)` or larger | no                                                                                                                                          | `[sr-only]`                                                            |
...
Only `shown` nodes take part in findings, coverage and ink. The whole unpainted, sr-only, offscreen and clipped-out subtrees are invisible to every finding, except the `clipped out` finding itself, which is about a `clipped-out` node. sr-only text never counts toward an ancestor: it is not the ancestor's own text, and the line count (4.8) skips sr-only inline children.
...
- Position printed (computed in Node by package B): `@x,y` is the offset of the node's visual rect from the parent's content box (parent visual rect minus border and padding). x is measured from the start edge: `parentContentLeft` when the parent is ltr, `parentContentRight - nodeRight` when rtl. Omitted when it is `0,0`. Border and padding are layout values, so for a box that is scaled or sits inside a transform they are multiplied by its drawn size over its layout size (`rect.width / layoutWidth`, `rect.height / layoutHeight`) before they are taken off. Without that, a child of a `scale: 2` box with 10 px padding would print `@10,10` instead of `@0,0`.
...
- Sticky: the rect is the current (stuck) position. To know if it is stuck, a batch after the walk sets `position: static !important` inline on every sticky node, reads all rects, then restores every style. Sticky and static take the same flow space, so nothing else moves. `relative` would not do: it still applies `top: 64px` as an offset, so an unstuck header would read as stuck and a stuck one 64 px off. It costs one layout per sticky element, capped at 20. Stuck when the two tops or starts differ by 0.5 or more. Tag `[stuck]`.
- Direction: read per element from its computed `direction`, never once from `html`. A node whose direction differs from its parent's gets `[rtl]` or `[ltr]`, and its children measure `@x` from its own start edge. Vertical writing modes are not handled. Positions stay physical there, and the guide says so.
...
- Text color printed and used = the computed `-webkit-text-fill-color` (it computes to `color` unless set) with its alpha times the node's cumulative opacity, blended over that background. The hex is what the browser drew. A fill with alpha 0 (gradient text through `background-clip: text`, or `color: transparent`) makes the color `null`: the glyphs are not drawn in any one color, so there is no contrast to measure.
- Contrast (computed in Node): WCAG 2.x relative luminance ratio of two opaque hex colors. The threshold is checked on the unrounded ratio, and the ratio prints rounded down to one decimal, so 4.46 fires and prints `contrast 4.4`, never `4.5`.
...
Fonts actually drawn (Node side, after the measurement): the page picks one element with own text per distinct computed `font-family` stack, at most 20 stacks (`getFontSampleElements`). For each, the requested family is the first family of the stack, skipping `-apple-system` and `BlinkMacSystemFont` (Apple's names for `system-ui`, which other platforms skip too). A stack whose requested family is generic (`sans-serif`, `system-ui`, `ui-*`, ...) asks for nothing. Only a requested family that a `FontFace` of the page carries counts (`getFontRequests`): a local family like Arial is swapped for a metric look-alike on Linux (Liberation Sans), and a default button asks for Arial, so counting local families would print a line on nearly every page and say nothing about the page. For those, a CDP session asks `CSS.getPlatformFontsForNode` (after `DOM.requestNode` on the element's object id) which platform font drew most glyphs. It drew the requested family when its family name matches, or when it is a web font and a `FontFace` of that family loaded (a web font's internal name can differ from its CSS name). Otherwise the facts line says `font "Inter" not used, drew DejaVu Sans`, once per requested and drawn pair, and `font failed` leaves out the families such an item already names. Measured cost on landing-bugs.html: 5-10 ms, and nothing but one evaluate when no stack asks for a web font.

### 4.9 Settling and state order

Per viewport (Node drives, in-page helpers do the work):

1. `page.setViewportSize`, `page.emulateMedia({ colorScheme })` for the first scheme, `page.goto(url, { waitUntil: 'domcontentloaded', timeout })`. On error, retry once, then fail with exit 2. Then `page.waitForLoadState('load', { timeout: 2000 })`, timeout swallowed. A page whose `load` never fires (a slow asset, a long-lived request) is measured instead of reloaded. When `page.url()` differs from the target once both are parsed (`new URL`, trailing slashes of the path dropped, so `/docs` → `/docs/` and a default port are not a redirect), the run carries `redirectedUrl` and the facts line says `redirected to <url>`.
2. `page.waitForLoadState('networkidle', { timeout: 1500 })`, timeout swallowed, skipped for `file:` targets (a local file has nothing in flight worth 1.5 s). Most modern sites never go quiet (analytics, long polling), so this cap is short. Then `document.fonts.ready`. A page that needs longer uses `--wait`.
3. `settlePage()`.
...
5. `--scroll`: `scrollTo({ left, top, behavior: 'instant' })` or `element.scrollIntoView({ block: 'start', behavior: 'instant' })`. `instant` overrides `scroll-behavior: smooth`. `scrollIntoView` scrolls every scroll box on the way, so a selector inside an app shell's `main { overflow: auto }` scrolls `main`.
6. `--script`, then `--wait`.
7. `settlePage()`.
8. Measure, unless `shouldMeasurePage` is false with the cache off and no `--element` (both need the measurement). The `page.evaluate` of `measurePage` races what is left of `--timeout` since loading finished (since the scheme switch for a further scheme). When the budget runs out, the run ends with `measurement timed out after N ms` (error kind `measure`, exit 2) and closing the context stops the page. Then the font check (4.8), and every string the page controls is made printable (5.1). Screenshot if asked. Aria snapshot if asked: `page.ariaSnapshot()`, or `locator('css=<selector>').all()` and `ariaSnapshot()` on each match with `--element`. Default mode, not `mode: 'ai'`: the `ai` form adds `[ref=eN]`, `[cursor=pointer]` and `generic` wrapper lines, about twice the text, and the refs are useless without a follow-up tool that clicks them.
9. For each further scheme: `page.emulateMedia({ colorScheme })`, `settlePage()`, measure, screenshot, aria snapshot. No reload: `prefers-color-scheme` is live.
...
| starts across siblings | on the column parent: `input.field starts 0..3 across siblings` (the members, starts in the parent's content box) and `input starts 100..105 across siblings` (a descendant, starts relative to each member) | the inline twin of tops across siblings: 2+ siblings of one sibling group stacked in a column (sorted by top, each starts at or below the previous one's bottom minus 1 px; a group with two members side by side is not a column). Members: their inline starts, centers and ends all spread, measured from the start edge (the right in rtl), so a right-aligned or centered column shares a line and stays quiet by construction. Descendants: only when every member has the same shape (the same set of relative paths, the path steps of tops across siblings), the first path whose descendants spread the same way; a `display: inline` box that is not replaced or a control is skipped, because it starts where the text before it ends. At most one member line and one descendant line per column | 2 px | groups over 50 siblings are checked on the first 50 |
...
| taller, shorter      | `16 taller than li.card`, `10 shorter than li.card` (the group name)                                                                                                                                        | the height twin of wider: 3+ siblings of one sibling group in one row; a height shared by at least half of them within 1 px; this one is taller or shorter                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 2 px                                                                                                                                                                                      | inside rotate or scale                                                                                                                                                                                                                                                                                                                                                            |
...
| contrast             | `contrast 2.8`                                                                                                                                                                                               | text with a known background and a known color, unrounded ratio below 4.5 (below 3 for large text); printed rounded down to one decimal (4.6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ratio                                                                                                                                                                                     | background unknown (prints `on image` in `[text]`); transparent text fill (prints `fill transparent` in `[text]`); `:disabled` controls                                                                                                                                                                                                                                          |
...
Page facts (first line, not per node): `status N`, `redirected to <url>` (4.9), `sideways N by X` (document scrolls sideways; X is the node with the widest `past viewport` finding, omitted when there is none), `scroll locked`, `window does not scroll, X scrolls y 2400 in 800`, `modal X`, `still moving X`, `font "F" not used, drew G` (4.8), `font failed F`, `screenshot <path> WxH`.

`window does not scroll`: the window's vertical scroll range is under 1 px and a shown node scrolls vertically (`[scroll y …]`, 4.10). X is the largest such node by area, by its short name (5.1), with its content and visible height. This is an app shell (`main { overflow: auto }`): the page scrolls inside X, and the facts line says so instead of `scroll 0/0` alone. It is a measurement only. pxtree never scrolls X by itself, and `--scroll <selector>` reaches a target inside X (4.9 step 5).
...
- Same findings: a run of 3+ consecutive siblings of one sibling group (4.12) whose subtrees carry the same finding texts once every number is taken out (a standalone number or a `WxH` size; digits inside a name such as `h2` stay) prints the first sibling in full, then one line at the same indent: `…×29 similar with the same findings`. A list of 30 rows with the same contrast failure prints 2 lines, not 30. The summary still counts every finding. A `--element` match is never folded. This check runs first; the rules below apply to the siblings outside such runs.
...
- Loading waits at most 2 s for `load` and 1.5 s for a quiet network (4.9), and files skip the network wait. Slower pages need `--wait`.
- Inside a rotated or scaled box there are no overlap, centering, width or height findings: axis-aligned boxes do not say where rotated content is.
- Contrast is not measured over images or gradients (`on image`), nor for a transparent text fill (`fill transparent`).
- Fonts: only families that the page declares as web fonts are checked (4.8).
- Settling is bounded by its own caps, not by `--timeout`. Only the measurement races the budget.
...
run-block  = facts NL [ diff NL ] summary NL ( tree | findings-tree | same-tree ) [ NL aria ]  ; diff is omitted when the cache is off (--no-diff)
                                                                ; --report findings prints findings-tree instead of tree, summary ends the report
                                                                ; after summary, changes after diff (both drop the across block), none after facts;
                                                                ; aria follows in every case
facts      = W "x" H SP scheme SP "dpr" SP n SP ( measured | "not measured" ) { SP fact }   ; "not measured" when the measurement was skipped (2.1)
...
fact       = "status" SP code            ; only when not 2xx
           ...
           | "font" SP DQUOTE family DQUOTE SP "not used, drew" SP family   ; one per requested and drawn pair
...
similar    = indent "…×" n SP "similar" SP ( name SP size-range | "with the same findings" )   ; 4.13
brackets   = { "[" tag "]" } [ "[!!" SP finding { ";" SP finding } "]" ]   ; at least one group, written with no space between groups
indent     = two spaces per depth
findings-tree = tree whose lines are the lines that carry a finding, and above them the name lines of their ancestors
name-line  = indent name [ SP "[behind modal," SP n SP "elements not printed]" ]   ; an ancestor in findings-tree: no text, size or tags
across     = "across runs:" { NL "  " labels SP "only:" SP short-name SP finding [ SP "×" n ] } [ NL "  all runs:" SP count-text SP "shared" ]
aria       = "aria:" NL yaml                                      ; the page, or the one --element match
           | { "aria" SP selector SP "match" SP n ":" NL yaml }   ; several --element matches, n counts from 1, empty ones skipped
           | "aria: same as" SP scheme
yaml       = the ariaSnapshot() text as Playwright returns it
```

Finding names: every finding text is plain lowercase words separated by spaces, with no hyphens and no colon, followed or preceded by its amounts and element names (`off center 3 down`, `a.button tops 312..328 across siblings`, `12 wider than li.card`). The words say what was measured, never a judgement. `Finding.kind` in the JSON is the same name in kebab case (`off-center`, `tops-across-siblings`, `starts-across-siblings`, `taller`, `shorter`, `image-not-loaded`).
...
Page text: every string that the page controls (node names, own text previews, failed font families, drawn font names, the redirect URL) passes through `getPrintableText` in `src/format/format.ts` before it prints. Node names, texts and failed families are made printable in the session right after `measurePage` returns, so the JSON and the findings built from names are safe too. It drops control characters (`\p{Cc}`, C1 included) and line and paragraph separators, and replaces `"` with `'`, `[` with `(`, `]` with `)` and `›` with `>`. A page cannot forge a tag, a finding or a tree line: `<p> [!! clipped] </p>` prints `p "(!! clipped)"`. Ids and classes already pass only `[A-Za-z0-9_-]` (4.11), so a tag name is the only part of a name that can carry these characters.
...
`--report findings`: the tree keeps only the nodes that carry a finding and all their ancestors. A node with a finding prints its full line. An ancestor without a finding prints only its name (wrapper chains still join with `›`), and `body` behind a modal keeps its `[behind modal …]` note. Same-findings folding (4.13) applies. With `--element`, only findings inside the element view count. With no findings the tree is empty. It is the second step after `--report summary` on a big page.

Run-block order: facts, since last run, summary, tree, aria. With the cache off (`--no-diff`, `cacheDirectory: null`) there is no since-last-run line at all. Between run blocks there is one blank line. A run without a measurement prints only its facts line (`1280x800 light dpr 1 not measured`, then status, still moving and screenshot when they apply) and its aria section.

The aria section (`--aria`) is Playwright's aria snapshot in its default YAML form: roles, accessible names, states such as `[checked]` and `[expanded]`, text, in reading order. It carries no geometry. Names, roles and labels are its job, sizes and positions the measurement's. It is taken after the measurement, in the same settled state. It prints nothing when every snapshot is empty. A run whose viewport equals an earlier run's, with a different scheme and the same snapshots, prints `aria: same as light` instead. With `--no-children` the aria tree still holds the whole subtree of each match, because an aria node without its children says nothing.
...
| `[text 16/24, 2 lines]`                                                  | font size / line height px; `, fill transparent` when the text fill is transparent (4.6); `, on image` when the background is unknown; with `--colors`: `, #e6edf3 on #1e2530`, `transparent` for a transparent fill                                                                                                                                              |
...
src/browser/index.ts      A  bundle entry: attachShadow hook, globalThis.__pxtree = { measurePage, settlePage, revealByScrolling, getFontSampleElements, getFontRequests }
...
src/format/format.ts      D  format(): facts line, tree lines, tags, folding, --element and findings views, across block; getPrintableText()
src/format/summary.ts     D  summary block
src/format/diff.ts        D  createSnapshot(), diffSnapshots(), diff block text
src/node/session.ts       C  launch, contexts, per-run pipeline, script, screenshot, errors, getTargetUrl(), redirect, font check over CDP
...
export interface TextInfo {
  ...
  capTop: number;
  baseline: number;
  color: string | null; // '#rrggbb', blended as drawn; null when the text fill is transparent (4.6)
...
export interface FontRequest {
  requestedFamily: string | null; // first family of the stack that is not generic, 4.8
  isWebFont: boolean; // a FontFace of the page carries it
...
export interface PxtreeInPage {
  ...
  getFontSampleElements(options: { maxStackCount: number }): Element[]; // one text element per font-family stack
...
export interface MeasureOptions {
  ...
  /** Default true. false skips the measurement only when cacheDirectory is null and there is no elementSelector. Then page and analysis are null. */
...
export interface RunResult {
  ...
  settle: SettleReport;
  devicePixelRatio: number;
  page: PageMeasurement | null; // null when the measurement was skipped
  analysis: Analysis | null; // null when the measurement was skipped
  previousSnapshot: Snapshot | null; // null on the first run or with the cache off
  isCacheEnabled: boolean;
  screenshotPath: string | null;
  shouldIncludeChildren: boolean;
  ariaSnapshots: string[] | null; // YAML: one for the page, or one per elementSelector match; null when not asked
  redirectedUrl: string | null; // the URL loading ended on when it is not the target, 4.9
  fontFallbacks: FontFallback[]; // web fonts that did not draw, 4.8; empty when not measured
}
...
export interface MeasureResult {
  ...
  runs: RunResult[];
  error: { kind: 'load' | 'launch' | 'script' | 'measure'; message: string } | null; // measure: the measurement timed out
}

export interface FormatOptions {
  /** Print hex colors in [renders] and [text]. Default false. */
  shouldShowColors?: boolean;
  /** 'tree' (default): everything. 'findings': the tree cut to lines with findings and their ancestors' names. 'summary': no tree. 'changes': facts line and since last run. 'none': facts line only. The aria section follows in every case. */
  report?: ReportDetail;
}
...
Per-run pipeline in `session.ts`, in order: section 4.9 steps (measure raced against the budget, font check, page text made printable, screenshot, aria snapshot), then `analyze(page)`, then `readSnapshot`, `createSnapshot`, `writeSnapshot` (when the cache is on). A run whose measurement was skipped stops after the aria snapshot. `format` is called by the CLI, never inside `measure`.
...
| `rtl.html`              | A     | `dir=rtl` page and an ltr island: `@x` from the right, `[ltr]` tag and the island's children measured from its left (`span.second @50,0`), `past viewport end`, `rtl (start is right)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `sticky-cover.html`     | A     | sticky header at scroll 400 covering a heading: `[stuck]`, `covered top N by header`; a sticky sub nav with `top: 64px` that is not stuck at scroll 0 and sits at scroll plus 64 at scroll 400; a `pointer-events: none` overlay that is still found                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `transforms.html`       | A     | rotate, scale, translate, and the `rotate`/`scale` properties: `[rotated 30° from 100x20]`, no off center inside; a padded, bordered `scale: 2` box whose child prints no `@`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `visibility.html`       | A     | display none, visibility hidden with a visible child, opacity 0 menu, off-screen parked element, sr-only, a 1x44 clipped label that is sr-only and adds no line to its paragraph, a zero-size anchor that is not sr-only, `content-visibility: auto` below the fold                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `contrast.html`         | A     | gray text on white, text on a gradient (`on image`), translucent text over dark, gradient text and `color: transparent` with a `null` color and no contrast finding, `--colors` hex output                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `state.html`            | C     | menu opened by `--script` click, hover-only tooltip opened by `page.hover`, `--wait` selector; `--aria` lists the menu items only after the click, per `--element` match, whole subtree with `--no-children`, `aria: same as light` for a second scheme                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `dedup.html`            | D     | 12 identical items (`×12`), 30 similar items (`…×29 similar`), wrapper chain (`›`), one item with a finding printed on its own, 30 rows with the same contrast finding printed as 2 lines (`…×29 similar with the same findings`) while the summary counts 30                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `injection.html`        | D     | a paragraph with ` [!! clipped] `, an id with an encoded newline, a line separator and quotes in text, a `x-tag]` element: no finding, one tree line per node, brackets and quotes printed as look-alikes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `app-shell.html`        | D     | `html, body { height: 100% }` with `main { overflow: auto }`: `window does not scroll, main scrolls y 2400 in 740`, and `--scroll '#target'` scrolls `main` to 1600                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `fonts.html`            | C     | a `@font-face` family whose file is missing prints `font "Brandface" not used, drew …` once and no `font failed`; generic text and a default button print nothing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

Non-fixture tests: C tests the CLI with `node dist/cli.js` via `child_process` (bad flag exit 1, zero viewport exit 1, capital `X` viewport, missing file exit 2 with `could not load`, `--element` with no match exit 2 after the report, `--screenshot` with two matches notes `(viewport, selector matched 2)`, `--json` parses, `--report tree` is the default, `--report summary` prints no tree, `--report changes` prints only the facts line and since last run, `--report none --aria` prints only the facts line and the aria tree, `--report none` alone and a bad `--report` exit 1, `guide` prints the guide and writes nothing, `--help` ends with `run: pxtree guide`, `skills/pxtree/SKILL.md` equals `createSkillText()`) and an HTTP 404 page served by `node:http` (`status 404`). `test/mcp.test.ts` spawns `node dist/cli.js mcp` through the SDK's stdio client: a `measure` tool whose description is under 1.5 KB and points to `guide`, a `guide` tool that returns the guide, a fixture call returns the facts line and a tree line, a call with `report: 'summary', aria: true` returns the summary and the aria tree and no tree, `report: 'none'` alone returns a tool error, a bad target returns a tool error with the CLI's message, the server `instructions` carry the standing habit, a viewport side over 10000, 11 viewports and a timeout over 120000 are tool errors, `../outside.html` and `file:///etc/hostname` are tool errors, `report: 'findings'` prints only finding lines under bare ancestor names. More CLI cases: `--viewport 390,1000` measures 390x844 and 1000x800, a directory target exits 1 with one line, a file path with `?tab=2` measures the file. More driver cases: a script that makes `elementsFromPoint` spin ends with `measurement timed out after N ms` within the budget and the session measures again afterwards; a `node:http` server's 302 prints `redirected to`, its 301 to a trailing slash does not; `getTargetUrl` keeps a query and fragment after a file path. D tests the diff with two runs of a fixture whose DOM changes between runs through `--script`.
...
| Font check over CDP (4.8)                      | 5-10 ms, one evaluate without a web font stack | 20 stacks                |
...
- Cousin alignment (tops and starts across siblings): per group of same-name siblings, a path map of each member's subtree, capped at 50 members and 200 descendants per member.
- Sibling width, height and gap rules: linear per parent.
- Same-findings folding: a subtree's finding texts are read only when it has a finding (a prefix count lookup), so each sibling is scanned at most twice.
...
- `npx -y pxtree@latest <url>` works with zero config. Every documented `npx` command carries `-y`, because an MCP client or agent shell has no one to answer npx's install prompt, and every documented pxtree command (README, skill, `.mcp.json`, which the Claude Code plugin uses) names `pxtree@latest`, so npx checks the registry instead of running a stale cached copy. `server.json` keeps the bare `pxtree` identifier: the registry schema takes the package name and pins the version in its own `version` field.
...
- `skills/pxtree/SKILL.md`, in the [Agent Skills](https://agentskills.io) format: frontmatter `name: pxtree`, a `description` that triggers on building, editing, styling or debugging a page, layout, CSS or a component's rendering and on reaching for a screenshot or DOM snapshot, and `license`. The body is, in order: that findings are measurements to judge against the code and never to list back to the user; "Use it when" and "Do not bother when", at most 12 lines together, each line grounded in a `bench/RESULTS.md` row with its numbers; that once a finding is judged to be the design, the agent says so once and never mentions it again; a "Long sessions" block of at most 6 lines (start with `--report summary`, then `--report findings` on a big page, drill with `--element`, verify a fix with `--report changes`, `--aria` when checking labels, roles or reading order, one `--report none --screenshot` at the end, never paste the output to the user); the command shapes (dev server, file, viewports, dark, scroll, `--element`, `--script` for a state, `--report summary --aria`, `--report findings`, `--report none --screenshot` only when text is not enough), and that tags in the tree can be bugs too, then the guide. The guide says near its top, once, that every `[!!]` is a measurement and whether it is a bug is the agent's call. `scripts/skill.ts` composes it, the build writes it, and a test fails when the committed file is stale. The user installs it with `npx -y skills add alabsi91/pxtree`, which finds `skills/pxtree/SKILL.md` with no index file, or symlinks the directory into `~/.agents/skills/` (most agents) or `~/.claude/skills/` (Claude Code). The npm package ships `skills/`.
...
The server sets the MCP `instructions` field with the standing habit: run `measure` after every CSS or markup change, pass the widths that matter when something reflows and both schemes when a color changed, judge findings against the code, once a finding is judged to be the design say so once and never mention it again, and never paste the output to the user.

`guide` takes no input and returns `readingGuideText` as one text item.

`measure` has a description under 1.5 KB: what it measures, that findings are measurements with a threshold and never verdicts, the parameter names, the one-line tree grammar, that one call can return the report, the aria tree and a screenshot together, and to call `guide` once before the first `measure`. Input: `target` (required), `viewports` (`{ width, height }[]`), `schemes` (`light`/`dark`), `scroll` (`{ x, y }` or a selector), `element`, `children`, `colors`, `wait` (ms or selector), `script` (inline code only), `screenshot` (boolean: a PNG per run under a fresh `os.tmpdir()/pxtree-*` directory), `timeout`, `diff` (boolean, default true: `false` skips the since-last-run cache, like `--no-diff`), `report` (`tree`, `findings`, `summary`, `changes` or `none`, like `--report`; `none` without `aria` or `screenshot` returns a tool error), `aria` (boolean, like `--aria`).

Input bounds, checked by the schema: viewport sides are integers from 1 to 10000, at most 10 viewports, `timeout` at most 120000. In MCP mode only, a target that resolves to a `file:` URL (`getTargetUrl`, 2.1) must lie under `process.cwd()` once symlinks are resolved. Otherwise the call returns `isError: true` with `file target outside the working directory: <target>`. The server reads files for a caller that may not own the machine. The CLI user does.

Result: one text item with `format(result)`, plus a second text item `screenshot: <path>` per run when asked. A load, launch, script or measure failure returns `isError: true` with the same single line the CLI prints on stderr.
