...
body 1280x850 [pad 24][gaps 24 24 24 24 16, free 16 at end]
  ul.cards 1232x208 [gaps across 16, free 600 at end][!! a.button tops 80..152 across siblings]
    ...
    …×2 similar li.card 200x136..200x208
...
|---|---|---|
| Codex | `codex mcp add pxtree -- npx -y pxtree mcp` | user, `~/.codex/config.toml` |
| Claude Code | `claude mcp add --scope user pxtree -- npx -y pxtree mcp` | user. `--scope project` writes `.mcp.json` |
| Cursor | [![Install in Cursor](https://cursor.com/deeplink/mcp-install-dark.svg)](https://cursor.com/install-mcp?name=pxtree&config=eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsInB4dHJlZSIsIm1jcCJdfQ%3D%3D) or `~/.cursor/mcp.json` / `.cursor/mcp.json` | user / project |
| VS Code Copilot | [![Install in VS Code](https://img.shields.io/badge/VS_Code-Install_Server-0098FF?style=flat-square)](https://insiders.vscode.dev/redirect?url=vscode%3Amcp%2Finstall%3F%257B%2522name%2522%253A%2522pxtree%2522%252C%2522command%2522%253A%2522npx%2522%252C%2522args%2522%253A%255B%2522-y%2522%252C%2522pxtree%2522%252C%2522mcp%2522%255D%257D) or `code --add-mcp '{"name":"pxtree","command":"npx","args":["-y","pxtree","mcp"]}'` | user. `.vscode/mcp.json` for project, see below |
...
```json
{
  ...
  "mcp": {
    "pxtree": { "type": "local", "command": ["npx", "-y", "pxtree", "mcp"], "enabled": true }
  }
}
```
...
{
  "mcpServers": {
    "pxtree": { "command": "npx", "args": ["-y", "pxtree", "mcp"] }
  }
}
```
...
| Gemini CLI | `gemini mcp add -s user pxtree npx -y pxtree mcp` | user. Without `-s user`: project `.gemini/settings.json` |
| Devin (formerly Windsurf) | `devin mcp add pxtree -- npx -y pxtree mcp`, files `~/.config/devin/mcp_config.json` / `.devin/mcp_config.json` | user / project |
| Amp | `amp mcp add pxtree -- npx -y pxtree mcp`, files `~/.config/amp/settings.json` / `.amp/settings.json`, see below | user / project |
...
{
  "servers": {
    "pxtree": { "type": "stdio", "command": "npx", "args": ["-y", "pxtree", "mcp"] }
  }
}
```
...
{
  "context_servers": {
    "pxtree": { "command": "npx", "args": ["-y", "pxtree", "mcp"], "env": {} }
  }
}
```
...
```
npx -y pxtree https://example.com --viewport 390x844,1280x800 --scheme light,dark
npx -y pxtree localhost:5173 --scroll '#pricing' --element '.card'
npx -y pxtree ./dist/index.html
npx -y pxtree localhost:5173 --script "await page.click('text=Menu')" --wait '.menu'
npx -y pxtree localhost:5173 --element '#pricing' --screenshot pricing.png
```
...
`npx -y pxtree --help` lists the flags. `npx -y pxtree guide` explains how to read the output.
...
- Contrast is not measured over images or gradients. The `[text]` tag says `on image`.
...
- Loading waits at most 2 s for `load` and 1.5 s for a quiet network. Slower pages need `--wait`.
