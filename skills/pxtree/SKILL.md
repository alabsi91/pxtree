...
Its findings are measurements, not verdicts. Judge each one against the code as "Findings are facts" in the guide below says, and never list them back to the user. It reports what the browser drew, which the source cannot tell you.
...
- tokens: about 540 a run, about 60 with --summary, against about 1100 for a 1280x800 screenshot
...
Long sessions:
- Start with `--summary`: facts, since last run and findings, no tree.
- Drill into one area with `--element '<selector>'`.
- After a fix, verify it with `--changes`: only the facts line and what changed since the last run.
- Take one `--screenshot` at the end, only if the text leaves a doubt.
- Never paste the tool output to the user.

Command shapes:

```
npx -y pxtree localhost:5173                                  # dev server
npx -y pxtree ./dist/index.html                               # HTML file
npx -y pxtree localhost:5173 --viewport 390x844,1280x800      # mobile and desktop
npx -y pxtree localhost:5173 --scheme dark                    # dark mode
npx -y pxtree localhost:5173 --scroll '#pricing'              # a region below the fold
npx -y pxtree localhost:5173 --element '.card'                # one component and its ancestors
npx -y pxtree localhost:5173 --script "await page.click('text=Menu')" --wait '.menu'   # a state: open menu, dialog, hover
npx -y pxtree localhost:5173 --screenshot shot.png            # only when the text is not enough
```

Tags in the tree such as `[clipped out by …]`, `[not painted …]` and `[children skipped …]` are measurements too, so once `--summary` points you somewhere, read the tree there before acting. Fix what the code shows is a bug, run again, and check `since last run` shows the change you meant.
